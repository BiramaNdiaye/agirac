...
export default function Index({ auth, visites, counts, currentFilter, search: initialSearch = '' }) {
    ...
    useEffect(() => {
        ...
        const increments = {
            ...
            delivered: counts.delivered / steps,
        ...
        const interval = setInterval(() => {
            ...
            setAnimatedCounts({
                ...
                delivered: Math.min(Math.floor(increments.delivered * currentStep), counts.delivered),
    ...
    // Fonction pour déterminer le statut et sa priorité (0 = En attente, priorité la plus haute)
    const getStatusPriority = (lastActionType) => {
        if (!lastActionType) return 0;
        switch (lastActionType) {
            case 'planification': return 1;
            case 'livraison_crvt': return 2;
            case 'impossibilite': return 3;
            case 'modification_date': return 4;

            default: return 0; // En attente 
        }
    };

    // Tri des visites : les "En attente" en haut, puis par priorité et enfin par ID récent
    const sortedVisites = useMemo(() => {
        return [...visites].sort((a, b) => {
            ...
            // Optionnel : second tri par date de création (du plus récent au plus ancien)
    ...
    const getStatusConfig = (lastActionType) => {
        const config = {
            ...
            'impossibilite': {
                label: 'Impossible',
                color: 'bg-gradient-to-r from-red-500 to-rose-600 text-white shadow-lg shadow-red-500/20',
                icon: <IconAlert />,
                badgeColor: 'bg-rose-100 text-rose-800 border-rose-200'
            },

            'modification_date': {
                label: 'Planif Modifié',
                color: 'bg-gradient-to-r from-green-500 to-green-600 text-white shadow-lg shadow-red-500/20',
                icon: <IconAlert />,
                badgeColor: 'bg-green-100 text-white border-green-200'
            },
        };
    ...
    const changeFilter = (filter) => {
        router.get('/visites-techniques', { filter, search }, { preserveState: true });
    };
    ...
    const tabs = [
        { key: 'all', label: 'Toutes', count: counts.all, icon: <IconTrendingUp />, color: 'from-slate-600 to-gray-700' },
        { key: 'planned', label: 'Planifiées', count: counts.planned, icon: <IconCalendar />, color: 'from-emerald-500 to-green-600' },
        { key: 'delivered', label: 'CRVT livré', count: counts.delivered, icon: <IconCheckCircle />, color: 'from-emerald-600 to-teal-600' },
        { key: 'impossible', label: 'Impossibilités', count: counts.impossible, icon: <IconAlert />, color: 'from-red-500 to-rose-600' },
    ];
        ...
        <Main user={auth.user}>
            <div className="relative min-h-screen overflow-hidden">
                ...
                <div className="absolute inset-0 -z-10 overflow-hidden">
                    ...
                    <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-amber-200 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse delay-700"></div>
                </div>
                ...
                <div className="relative z-10 py-8">
                    <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        {/* Cartes statistiques avec améliorations */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                            {tabs.map((tab, idx) => (
                                <div key={tab.key} 
                                    ...
                                    <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-white/40 shadow-xl overflow-hidden">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className={`p-3 rounded-xl bg-gradient-to-br ${tab.color} shadow-lg transition-transform group-hover:scale-110`}>
                                                {tab.icon}
                                            </div>
                                            <div className="text-xs font-mono text-slate-400 bg-white/50 px-2 py-1 rounded-full">
                                                {tab.label === 'Toutes' ? 'total' : tab.label.toLowerCase()}
                                            </div>
                                        </div>
                        ...
                        {/* Tableau modernisé avec tri par statut */}
                        <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl border border-white/40 shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">
                            <div className="overflow-x-auto">
                                <div className="inline-block min-w-full align-middle">
                                    <table className="min-w-full divide-y divide-slate-200">
                                        <thead className="bg-gradient-to-r from-slate-50 to-white backdrop-blur-sm">
                                            <tr>
                                                ...
                                                <tr>
                                                    <td colSpan="6" className="px-6 py-20 text-center">
                                                        <div className="flex flex-col items-center gap-4">
                                                            ...
                                                            <p className="text-slate-500 text-lg font-medium">Aucune visite technique trouvée</p>
                                                            <button onClick={() => { setSearch(''); changeFilter('all'); }} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-lg text-white transition-all shadow-md hover:shadow-lg">Réinitialiser la recherche</button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ) : (
                                                sortedVisites.map((visite, idx) => {
                                                    const { label, color, icon, badgeColor } = getStatusConfig(visite.last_action_type);
                                                    const isPending = getStatusPriority(visite.last_action_type) === 0;
                                                    return (
                                                        <tr key={visite.id} 
                                                            className={`group transition-all duration-300 hover:bg-slate-50/80 transform hover:scale-[1.01] ${isPending ? 'border-l-4 border-l-amber-400' : ''}`}
                                                            style={{ animationDelay: `${idx * 30}ms` }}>
                                                            <td className="px-6 py-4">
                                                                <div className="text-sm font-bold text-slate-800">{visite.admin_rds || '-'}</div>
                                                                <div className="text-xs text-slate-500 mt-0.5 font-mono">{visite.admin_bca || '-'}</div>
                                                            </td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-slate-600">
                                                                {visite.admin_contract || '-'}
                                                            </td>
                                                            <td className="px-6 py-4">
                                                                <div className="text-sm text-slate-700 font-medium">{visite.project_name || '-'}</div>
                                                                <div className="text-xs text-slate-500 mt-0.5">{visite.rop_ref || '-'}</div>
                                                            </td>
                                                             <td className="px-6 py-4">
                                                                <div className="text-sm text-slate-700 font-medium">
                                                                        {visite.one_shot_info === null || visite.one_shot_info === undefined ? (
                                                                            <span className="text-slate-400">-</span>
                                                                        ) : visite.one_shot_info ? (
                                                                        ...
                                                                        {visite.is_annule ? (
                                                                            <div>
                                                                                ...
                                                                                {visite.annulation_date && (
                                                                                    <div className="text-xs text-slate-500 mt-0.5">
                                                                                        le {visite.annulation_date}
                                                                                    </div>
                                                                                )}
                                                                            </div>
                                                                        ) : (
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    {visite.actions_count > 1 && (
                                                                        <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full" title="Nombre d'actions">
                                                                            {visite.actions_count}
                                                                        </span>
                                                                    )}
                                                                </div>
                                                             </td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                                <div className="flex items-center gap-3">
                                                                    <Link
                                                                        href={`/visites-techniques/${visite.id}`}
                                                                        className="inline-flex items-center gap-1 text-emerald-600 hover:text-emerald-800 font-bold transition-all duration-300 hover:gap-2 group/btn"
                                                                    >
                                                                        <IconEye />
                                                                        <span>Détails</span>
                                                                    ...
                                                                    <Link 
                                                                        href={route('suivi.show', visite.admin_rds)} 
                            ...
                            <div className="bg-slate-50/80 px-6 py-4 border-t border-slate-200 flex justify-between items-center flex-wrap gap-2">
                                <span className="text-slate-600 text-sm">Total : <span className="font-bold text-slate-800">{sortedVisites.length}</span> visite(s) affichée(s)</span>
